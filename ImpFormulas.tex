...
\begin{enumerate}
	...
	\item Energy of a Wave(for a length $dx$ at $x$): $$dK=dE=\frac{1}{2}\mu \omega^2\|\psi\|^2 dx$$
...
\end{equation}
	...
	\item Relation between $Q$ and $E_{th}$:
		...
		\end{equation}
		where $m$ is the mass of incoming particle while $M$ is the mass of the particle at rest. (*Can be genralised by using $\frac{M+m}{M}$ and then interpreting some kind of total mass)
...
G* is calculated using an electrolyte with known conductivity
	...
	\item The meaning of Van Der Waal's constants:
		\begin{itemize}
			\item \emph{a}: It is the proportionality constant, and the $\left(\frac{n}{V}\right)^2$ comes from the fact that pressure depends on both force of repulsion and the frequency of collisions both of which are reduced by factors of concentration ($=\frac{n}{V}$)
	...
	\item Gases(ideal and real):
		\begin{enumerate}
			...
			\item Critical Volume:
				...
				\end{equation}
		\end{enumerate}
	\item $\mathrm{P_{\text {final }}=\sqrt{P_{x}^{o} \times P_{y}^{o}}}$
\end{enumerate}
\section{Math}
\begin{enumerate}
...
\end{equation}			
	...
	\item Taylor series of $\tan(x)$:\begin{equation} \tan(x)\approx x+\frac{1}{3}x^3+\frac{2}{15}x^5+\dots\end{equation}
		...
		\item Anytime you encounter an integral which involves something like $\frac{x^2-1}{x^2+1}$ try putting in $t=x+\frac{1}{x}$. for eg.:
			\begin{align*}
